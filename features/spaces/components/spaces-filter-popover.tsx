...
const TABS: { id: FilterTab; label: string; iconName: "calendar" | "loader" | "building2" }[] = [
  ...
  { id: "space",  label: "Space",  iconName: "building2" },
]

const empty = (): SpaceFilters => ({ statuses: [], spaceTypes: [], dateFrom: "", dateTo: "" })

function countActive(f: SpaceFilters) {
  let n = 0
...
export function SpacesFilterPopover({ value, onChange }: Props) {
  const [open, setOpen] = useState(false)
  const [activeTab, setActiveTab] = useState<FilterTab>("date")
  const [draft, setDraft] = useState<SpaceFilters>(value)
  ...
  const toggleStatus = (v: string) =>
    setDraft(p => ({ ...p, statuses: p.statuses.includes(v) ? p.statuses.filter(s => s !== v) : [...p.statuses, v] }))

  const toggleSpace = (v: string) =>
    setDraft(p => ({ ...p, spaceTypes: p.spaceTypes.includes(v) ? p.spaceTypes.filter(s => s !== v) : [...p.spaceTypes, v] }))
      ...
      >
        ...
        <div className="flex border-b items-center justify-between py-2">
          ...
          <div className="flex items-center gap-2">
            <Button type="button" size="sm" variant="secondary-outline"
        ...
        <div className="pt-2 flex pb-4">
            ...
            {TABS.map(tab => (
              ...
              >
                <Icon name={tab.iconName} size={16}
                  className={activeTab === tab.id ? "text-foreground" : "text-secondary-foreground"} />
                {tab.label}
              </button>
            ))}
          </div>
          ...
          <div className="flex-1 pr-2 sm:pr-5 pl-4 sm:pl-6 pt-1 min-w-0">
            ...
            {activeTab === "space" && (
              <div className="space-y-3">
                <p className="text-sm font-semibold text-foreground">Space type</p>
                <div className="space-y-2.5">
                  {SPACE_OPTIONS.map(opt => (
                    <label key={opt.value} className="flex items-center gap-2.5 cursor-pointer">
                      <Checkbox checked={draft.spaceTypes.includes(opt.value)} onCheckedChange={() => toggleSpace(opt.value)} />
                      <span className="text-sm text-foreground">{opt.label}</span>
                    </label>
                  ))}
                </div>
