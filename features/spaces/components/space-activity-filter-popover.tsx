...
interface SpaceActivityFilterPopoverProps {
  value: SpaceActivityFilters
  onChange: (filters: SpaceActivityFilters) => void
}

type FilterTab = "date" | "status" | "space"

function countActiveFilters(f: SpaceActivityFilters): number {
  let n = 0
...
const emptyFilters = (): SpaceActivityFilters => ({
  statuses: [],
  spaceTypes: [],
  dateFrom: "",
  dateTo: "",
...
const TABS = [
  ...
  { id: "space" as FilterTab, label: "Space", iconName: "building2" as const },
]

export function SpaceActivityFilterPopover({
  value,
  onChange,
}: SpaceActivityFilterPopoverProps) {
  const [open, setOpen] = useState(false)
  const [activeTab, setActiveTab] = useState<FilterTab>("date")
  const [draft, setDraft] = useState<SpaceActivityFilters>(value)
